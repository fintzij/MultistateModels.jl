...
x = [0.0, 0.3, 0.6, 1.0]
B = BSplineBasis(BSplineOrder(3), copy(x))
c = -rand(length(B))
S = Spline(B, c)
I = integral(S)

B4 = BSplineBasis(BSplineOrder(4), copy(x))

# evaluating the basis functions generated by I and B4 give the same result
# so, suffice to consrain the parameters. 

# why the scaling to go from the coefficients for B to I?