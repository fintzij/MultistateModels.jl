...
# set up model
model_sim = setup_model(; make_pars = true, data = nothing, family = "wei", ntimes = 12, nsubj = 250)
...
model_sim2 = setup_model(; make_pars = false, data = model_sim.data, family = "exp", ntimes = ntimes)

set_parameters!(model_sim2, model_fitted.parameters)
paths_sim = simulate(model_sim2; nsim = 10, paths = true, data = false)


asymp_cis = asymptotic_bootstrap(model_sim2, flatview(model_fitted.parameters), model_fitted.vcov,  sims_per_subj, nboot)    
get_estimates(paths_sim)
get_estimates(paths)
